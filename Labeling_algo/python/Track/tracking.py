...
clases={'particular': 0, 'bus': 1, 'motociclista': 2, 'minivan': 3, 'peaton': 4,  'camion': 5, 'taxi': 6, 'ciclista': 7, 'tractomula': 8}
...
class singlepath:
    ...
    def __init__(self, idx,x,y,w,h,framesttl,featurestring,lineasconteo=10):
        ...
        self.ttl=framesttl
        self.colour=(int(255/9*(1+clases[featurestring])))#,int(250),int(250))
    ...
    def insertToPath(self,idx,x,y,w,h,st,framesttl):
        ...
        for find in self.p[:]:
            if idx==find.idx:
                ...
                find.colour=(int(255/9*(1+clases[find.str])))#,int(250),int(250))
                return 0
